...
export const inviteUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Enforce field-level permissions (story 113): strip standard and custom
  // fields the viewer is not allowed to edit. Applies to create as well as
  // update — the frontend may be tampered with.
  ...
  let permissionedCustomFields = validatedCustomFields ?? {};
  if (Object.keys(permissionedCustomFields).length > 0) {
    permissionedCustomFields = await enforceCustomFieldPermissions(
      req.user.company_id,
      'user',
      req.user.userId,
      permissionedCustomFields,
    );
  }
...
export const updateUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Enforce field-level permissions (story 113): strip standard and custom
  // fields the viewer is not allowed to edit. This is the server-side
  // security boundary — the frontend may be tampered with.
  const filteredInput = await enforceStandardFieldPermissions(
    req.user.company_id,
    'user',
    req.user.userId,
  ...
  if (filteredInput.custom_fields !== undefined) {
    filteredInput.custom_fields = await enforceCustomFieldPermissions(
      req.user.company_id,
      'user',
      req.user.userId,
      input.custom_fields as Record<string, unknown>,
    );
  }

  // Normalize empty strings → null
  const updates: Record<string, unknown> = { ...filteredInput };
